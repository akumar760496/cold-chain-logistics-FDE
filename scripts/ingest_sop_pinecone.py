...
from faiss.loader import has_AVX2  # unused, kept as in original
from openai import embeddings     # unused, kept as in original
import pandas as pd
import pypdf
...
import os
os.environ["TOKENIZERS_PARALLELISM"] = "false"  # CRITICAL FIX
...
if EMBEDDINGS_MODEL_SETTINGS == "OPENAI":
    print("Mode: Utilizing Cloud based OpenAI Embeddings model")
    embeddings = OpenAIEmbeddings()
    INDEX_NAME = "fde-sop-index-openai"
    TARGET_DIMENSION = 1536
else:
    local_model_target = os.getenv("Local_Embedding_Model", "BAAI/bge-m3").strip()
    print(f"Mode: Local fallback Settings Activated. Launching [{local_model_target}]")
...
# Self-healing verification in case an index was created with wrong legacy dimensions
if INDEX_NAME in existing_index:
    ...
    if desc.dimension != TARGET_DIMENSION:
        ...
        existing_index = [name for name in existing_index if name != INDEX_NAME]
...
# Initialize clients AFTER ensuring index exists (fixes NameError)
...
# Robust Polymorphic Parser for Chunking Logic
def parse_and_chunk_document(doc_path: Path) -> list[Document]:
    ext = doc_path.suffix.lower()
    raw_chunks: list[Document] = []  # FIXED: was `raw_chunks = list[Document] = []`
    ...
    if ext == ".md":
            ...
            with open(doc_path, "rb") as f:
                ...
                for page_num, page in enumerate(reader.pages):
                    ...
                    if page_text and page_text.strip():
                        pdf_docs.append(Document(page_content=page_text, metadata={
                            "page_number": page_num + 1
                        }))
            raw_chunks = text_splitter.split_documents(pdf_docs)
        except Exception as e:
            print(f"Error parsing pdf file {doc_path.name}: {e}")
            return []
        ...
        for idx, row in df.iterrows():  # FIXED: added parentheses
            ...
            if row_items:
                ...
                doc_item = Document(page_content=row_text, metadata={
                    "row_index": int(idx)
                })
...
# Incremental PIPELINE with Batching
...
print(f"Found {len(current_files)} policy files in {policy_dir}...")
...
for deleted_file in deleted_files:
    print(f"Detected deleted files: {deleted_file}. Purging from Pinecone...")
    try:
...
for file_name, file_path in current_files.items():
    ...
    print(f"Processing updated: {file_name}....")
    cache_modified = True

    try:
        # Parse first to avoid deleting old data if parsing fails
        chunks = parse_and_chunk_document(file_path)
        if not chunks:
            print(f"No valid text chunks extracted from {file_name}..")
            continue  # do not update cache for this file

        # Now safe to delete old chunks
        try:
        ...
        print(f"Upserting {total_chunks} chunks in batches")
        ...
        print(f"Error during ingestion of file {file_name} : {e}")
        # do not add to updated_cache on failure
